...
def score_paper(paper: dict) -> int:
    ...
    # 5. Journal quality boost — tiered for materials science / thermal management direction
    journal = (paper.get("journal", "") or "").lower().strip()

    # Tier S: flagship journals (exact match, avoid matching "nature communications" etc.)
    flagship_journals = {"nature", "science"}
    # Tier A: Nature-family and Science-family journals
    nature_science_family = [
        "nature materials", "nature communications", "nature energy",
        "nature nanotechnology", "nature chemistry", "nature sustainability",
        "nature reviews materials", "nature physics", "nature chemical engineering",
        "science advances", "science robotics",
    ]
    # Tier B: other top-tier materials/energy/chemistry journals (broadly IF>10-15, aligned
    # with the user's target list: AM/AFM/NC and adjacent venues)
    top_materials_journals = [
        "advanced materials", "advanced functional materials", "advanced energy materials",
        "advanced science", "joule", "matter", "chem", "journal of the american chemical society",
        "angewandte chemie", "proceedings of the national academy of sciences", "pnas",
        "national science review", "energy & environmental science", "acs nano",
        "energy storage materials", "nano-micro letters", "nano energy",
        "chemical engineering journal", "small", "materials horizons",
    ]

    if journal in flagship_journals:
        score += 5
    elif any(j in journal for j in nature_science_family):
        score += 4
    elif any(j in journal for j in top_materials_journals):
        score += 2
